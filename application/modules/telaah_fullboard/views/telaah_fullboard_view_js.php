    ...
    <script type="text/javascript">
        ...
        $("#protokol1, #protokol2").select2({
          ...
          width: "100%"
    		...
    		$('#pilih1').click(function(){
    			...
    			window.location.href = '<?php echo base_url()?>telaah_fullboard/form/0/'+id_pep;
    		})
    		...
    		jQuery(grid_selector1).jqGrid({
    			...
    			subGrid : false,
    			url: '<?php echo base_url()?>telaah_fullboard/get_daftar1/',
    			datatype: "json",
          mtype: "POST",
    			height: 250,
    			colNames:['ID', 'ID PEP', 'No Protokol', 'Judul', 'Tanggal Pengajuan', 'KEPK', 'Waktu Mulai', 'Waktu Selesai', 'Tanggal Telaah Kelayakan', 'Kelayakan'],
    			colModel:[
    				...
    				{name:'kelayakan', index:'kelayakan', editable:false},
    			], 
    	    ...
    	    rownumbers: true,
    			...
    			ondblClickRow: function (rowid, iRow, iCol, e) {
    				...
    				window.location = '<?php echo base_url()?>telaah_fullboard/form/'+rowid+'/'+id_pep;
    			},
    		...
    		jQuery(grid_selector2).jqGrid({
    			...
    			subGrid : false,
    			url: '<?php echo base_url()?>telaah_fullboard/get_daftar2/',
    			datatype: "json",
          mtype: "POST",
    			height: 250,
    			colNames:['ID', 'ID PEP', 'No Protokol', 'Judul', 'Tanggal Pengajuan', 'Tanggal Perbaikan Protokol', 'Perbaikan ke', 'KEPK', 'Waktu Mulai', 'Waktu Selesai', 'Tanggal Telaah Kelayakan', 'Kelayakan'],
    			colModel:[
    				{name:'id', index:'id', hidden: true, editable:false},
    				{name:'id_pep', index:'id_pep', hidden: true, editable:false},
    				{name:'no_protokol', index:'no_protokol', editable:false},
    	    ...
    	    rownumbers: true,
    			...
    			caption: "Telaah Full Board (Perbaikan)"
    		...
    		jQuery(grid_selector1).jqGrid('navGrid',pager_selector1,
    			...
    			{
    				//delete record form
    				url: '<?php echo base_url()?>telaah_fullboard/hapus/',
    				recreateForm: true,
    		...
    		function editRow1(rowid) {
    			var id_pep = jQuery(grid_selector1).jqGrid('getCell', rowid, 'id_pep');
    			window.location.href = '<?php echo base_url()?>telaah_fullboard/form/'+rowid+'/'+id_pep;
    		}
    	...
    	var Protokol = function(id, no, judul, waktu, tgl_fb, jam_fb, tempat_fb, revisi_ke, hari_ke) {
    		this.id = id;
    		this.no = no;
    		this.judul = judul;
        this.tgl_fb = tgl_fb;
    	...
    	var TelaahFullBoardModel = function(){
    		var self = this;
    	...
    	var App = new TelaahFullBoardModel();
        ...
        $.ajax({
          ...
          success: function(res, xhr){
            $.each(res, function(i, item){
              App.data_protokol.push(new Protokol(res[i].id_pep, res[i].no, res[i].judul, res[i].tgl_fb, res[i].jam_fb, res[i].tempat_fb, res[i].waktu, res[i].revisi_ke, res[i].hari_ke));
            });
          }
        });
    	}
