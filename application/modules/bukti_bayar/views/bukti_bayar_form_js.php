    ...
    <script type="text/javascript">
      ...
      function getNewid()
    	...
    	var BuktiBayarModel = function(){
    		var self = this;
    		self.id = ko.observable(<?php echo isset($data['id_bb']) ? htmlentities($data['id_bb'], ENT_QUOTES) : 0 ?>);
    	...
    	var App = new BuktiBayarModel();

      App.showFile = function(path){
        sp = path.split('/');
        file_name = sp[1];
        $.ajax({
          url: '<?php echo base_url()?>bukti_bayar/cek_file_exist/'+file_name,
          type: 'post',
          cache: false,
          dataType: 'json',
      ...
      App.downloadFile = function(file_name, client_name){
        $.ajax({
          ...
          success: function(res, xhr){
            if (res.isSuccess)
            {
              window.location.href = '<?php echo base_url()?>bukti_bayar/download/'+rawurlencode(file_name)+'/'+rawurlencode(client_name);
            }
            else
      ...
      App.back = function(){
    		window.location.href = '<?php echo base_url()?>bukti_bayar/';
    	}
    	...
    	App.save = function(createNew){
    	  ...
    	  App.processing(true);
    	  $.ajax({
    	    url: '<?php echo base_url()?>bukti_bayar/proses',
    	    type: 'post',
    	    cache: false,
    	    dataType: 'json',
    	    data: data,
