    ...
    $('#add').click(function(){
      addRow();
		...
		jQuery(grid_selector).jqGrid({
			...
			subGrid : false,
			url: '<?php echo base_url()?>putusan_awal/get_daftar/',
			datatype: "json",
      mtype: "POST",
			height: 250,
			colNames:['ID', 'No Protokol', 'Judul', 'Tanggal Pengajuan', 'KEPK', 'Waktu Mulai', 'Waktu Selesai', 'Tanggal Putusan', 'Klasifikasi'],
			colModel:[
				...
				{name:'tgl_pa', index:'tgl_pa', editable:false, align:'center'},
	    ...
	    rownumbers: true,
			...
			ondblClickRow: function (rowid, iRow, iCol, e) {
				window.location = '<?php echo base_url()?>putusan_awal/form/'+rowid;
			},
			...
			caption: "Putusan Screening Jalur Telaah Protokol"
		...
		function editRow(rowid) {
			window.location.href = '<?php echo base_url()?>putusan_awal/form/'+rowid;
		}

		function addRow() {
			window.location.href = '<?php echo base_url()?>putusan_awal/form/';
		}
