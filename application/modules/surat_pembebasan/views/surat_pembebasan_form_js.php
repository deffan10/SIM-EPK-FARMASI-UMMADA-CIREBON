    ...
    <script type="text/javascript">
      ...
      var SKEP = function(id, no_tampilan, no, idx_std, parent, child, level, uraian, uraian_master, pil_pengaju, pil_sekretaris, just_header){
        this.id = id;
        ...
        this.pil_sekretaris = pil_sekretaris;
        this.just_header = just_header;
      }

    	var CetakModel = function(){
    		var self = this;
    		self.id = ko.observable(<?php echo isset($data['id_ethical_exemption']) ? $data['id_ethical_exemption'] : 0?>);
    	...
    	App.get_standar_kelaikan = function(){
    		var id_pep = App.id_pep() != '' ? App.id_pep() : 0;
    		$.ajax({
    			url: '<?php echo base_url()?>surat_pembebasan/get_standar_kelaikan/'+id_pep,
    			type: 'post',
    			cache: false,
    			dataType: 'json',
    			success: function(res, xhr){
    				if (res.length > 0)
    				{
    					...
    					$.each(res, function(i, item){
    						if (item.idx_std == 1){
    			     		App.self_assesment1.push( new SKEP(item.id, item.no_tampilan, item.no, item.idx_std, item.parent, item.child, item.level, item.uraian, item.uraian_master, item.pil_pengaju, item.pil_sekretaris, item.just_header) );
    						}
    						else if (item.idx_std == 2){
    			     		App.self_assesment2.push( new SKEP(item.id, item.no_tampilan, item.no, item.idx_std, item.parent, item.child, item.level, item.uraian,item.uraian_master, item.pil_pengaju, item.pil_sekretaris, item.just_header) );
    						}
    						else if (item.idx_std == 3){
    			     		App.self_assesment3.push( new SKEP(item.id, item.no_tampilan, item.no, item.idx_std, item.parent, item.child, item.level, item.uraian, item.uraian_master, item.pil_pengaju, item.pil_sekretaris, item.just_header) );
    						}
    						else if (item.idx_std == 4){
    			     		App.self_assesment4.push( new SKEP(item.id, item.no_tampilan, item.no, item.idx_std, item.parent, item.child, item.level, item.uraian, item.uraian_master, item.pil_pengaju, item.pil_sekretaris, item.just_header) );
    						}
    						else if (item.idx_std == 5){
    			     		App.self_assesment5.push( new SKEP(item.id, item.no_tampilan, item.no, item.idx_std, item.parent, item.child, item.level, item.uraian, item.uraian_master, item.pil_pengaju, item.pil_sekretaris, item.just_header) );
    						}
    						else if (item.idx_std == 6){
    			     		App.self_assesment6.push( new SKEP(item.id, item.no_tampilan, item.no, item.idx_std, item.parent, item.child, item.level, item.uraian, item.uraian_master, item.pil_pengaju, item.pil_sekretaris, item.just_header) );
    						}
    						else if (item.idx_std == 7){
    			     		App.self_assesment7.push( new SKEP(item.id, item.no_tampilan, item.no, item.idx_std, item.parent, item.child, item.level, item.uraian, item.uraian_master, item.pil_pengaju, item.pil_sekretaris, item.just_header) );
    						}
    			    });			
    				}
    			}
    	...
    	App.save = function(createNew){
    	  ...
    	  App.processing(true);
    	  $.ajax({
    	    url: '<?php echo base_url()?>surat_pembebasan/proses',
    	    type: 'post',
    	    cache: false,
    	    dataType: 'json',
    	    data: data,
    	...
    	App.back = function(){
    		window.location.href = '<?php echo base_url()?>surat_pembebasan/';
    	}

    	App.print = function(){
    		var id = App.id();
    		window.open('<?php echo base_url()?>surat_pembebasan/cetak_surat/'+id, '_blank');
    	}
    	...
    	App.print_sa = function(){
    		var id = App.id(), 
				...
				if (id > 0)
	    		window.open('<?php echo base_url()?>surat_pembebasan/cetak_sa/'+id+'/'+id_pep, '_blank');
				else
    		...
    		bootbox.confirm({
    			...
    			callback: function(result) {
    				if(result) {
    				  ...
    				  App.processing(true);
    				  $.ajax({
    				    url: '<?php echo base_url()?>surat_pembebasan/kirim/',
    				    type: 'post',
    				    cache: false,
    				    dataType: 'json',
    				    data: data,
    ...
    <?php if (isset($data['id_ethical_exemption']) && $data['id_ethical_exemption'] > 0) { ?>
    	App.get_standar_kelaikan();
    <?php } ?>
    </script>