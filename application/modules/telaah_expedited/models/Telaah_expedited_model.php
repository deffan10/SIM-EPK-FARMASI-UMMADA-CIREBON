...
class Telaah_expedited_model extends Core_Model {
  ...
  var $fieldmap_filter;
	...
	{
		$this->insert_telaah_expedited();
		$this->insert_expedited_self_assesment();
	}

	function insert_telaah_expedited()
	{
		$this->db->select('1')->from('tb_telaah_expedited')->where('id_texp', $this->data['id_texp']);
		$rs = $this->db->get()->row_array();

		if ($rs)
		{
			$this->db->where('id_texp', $this->data['id_texp']);
			$this->db->update('tb_telaah_expedited', $this->data);
			$this->check_trans_status('update tb_telaah_expedited failed');
			$this->id = $this->data['id_texp'];

			$aktivitas = 'Edit Telaah Expedited '.$this->input->post('no_protokol');
		...
		{
			unset($this->data['id_texp']);
			$this->db->insert('tb_telaah_expedited', $this->data);
			$this->check_trans_status('insert tb_telaah_expedited failed');
			$this->id = $this->db->insert_id();

			$aktivitas = 'Insert Telaah Expedited '.$this->input->post('no_protokol');
			$id_user_kepk = $this->get_id_atk_penelaah($this->session->userdata('id_user_'.APPAUTH));
			$id_user =$this->session->userdata('id_user_'.APPAUTH);
			simpan_logaktivitas_tim_kepk($aktivitas, $id_user_kepk, $id_user);
		}		
 	...
 	function insert_expedited_self_assesment()
	{
    $data_update = array();
    $data_insert = array();
		for ($i=0; $i<count($this->data_sa); $i++)
		{
			$this->db->select('id_esa')->from('tb_expedited_self_assesment')->where('id_texp', $this->id)->where('id_jsk', $this->data_sa[$i]['id_jsk']);
			$rs = $this->db->get()->row_array();
    ...
    if (!empty($data_update)){
      $this->db->update_batch('tb_expedited_self_assesment', $data_update, 'id_esa');
      $this->check_trans_status('update tb_expedited_self_assesment failed');
    }

    if (!empty($data_insert)){
      $this->db->insert_batch('tb_expedited_self_assesment', $data_insert);
      $this->check_trans_status('insert tb_expedited_self_assesment failed');
    }
	}
	...
	public function get_data_jqgrid($param, $isCount=FALSE, $CompileOnly=False)
  {
    ...
    // protokol yg expedited adalah protokol baru telah diputuskan awal sebagai expedited (klasifikasi=2) dan protokol perbaikan yg dikirimkan lagi setelah diperbaiki dan putusan awal sebelumnya expedited (klasifikasi=2)
		$query = "
      ...
      left join tb_telaah_expedited as te on te.id_pep = e.id_pep and te.id_atk_penelaah = ".$id_atk_penelaah."
      left join tb_putusan_expedited as pexp on pexp.id_pep = e.id_pep
			where p.id_kepk = ".$this->session->userdata('id_kepk_tim')."
				and pa.klasifikasi = 2
        and te.id_texp is null
				and pexp.id_pexp is null
        and e.revisi_ke = 0
      ...
      join tb_pengajuan as p on p.id_pengajuan = e.id_pengajuan
      join tb_kirim_ke_kepk as kr on kr.id_pengajuan = e.id_pengajuan and kr.id_pep = e.id_pep and kr.id_kepk = p.id_kepk
      join (select e2.id_pengajuan, pa2.inserted from tb_pep as e2 join tb_putusan_awal as pa2 on pa2.id_pep = e2.id_pep join tb_penelaah_mendalam as pm2 on pm2.id_pa = pa2.id_pa where pa2.klasifikasi = 2 and pm2.id_atk_penelaah = ".$id_atk_penelaah.") as x on x.id_pengajuan = p.id_pengajuan
      left join tb_telaah_expedited as te on te.id_pep = e.id_pep and te.id_atk_penelaah = ".$id_atk_penelaah."
      left join tb_putusan_expedited as pexp on pexp.id_pep = e.id_pep
      where p.id_kepk = ".$this->session->userdata('id_kepk_tim')."
    ...
    $revisi_ke = $rs['revisi_ke'];
		...
		$this->db->join('tb_expedited_self_assesment as tea', 'tea.id_jsk = jsk.id_jsk and tea.id_texp = '.$id, 'left');
		$result = $this->db->get()->result_array();
	...
	{
		$this->db->select('1')->from('tb_putusan_expedited')->where('id_pep', $id_pep);
		$rs = $this->db->get()->row_array();
	...
	{
		$this->db->select('pe.ringkasan');
		$this->db->from('tb_putusan_expedited as pe');
		$this->db->join('tb_pep as e', 'e.id_pep_old = pe.id_pep');
		$this->db->where('e.id_pep', $id_pep);
		$result = $this->db->get()->row_array();
	...
	function get_data_catatan_before($id_pep, $id_pengajuan, $catatan)
  {
    ...
    $this->db->select('te.id_pep, te.'.$catatan.' as catatan, te.inserted');
