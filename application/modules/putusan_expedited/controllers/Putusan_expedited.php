...
class Putusan_expedited extends Userpage_Controller {

  public function __construct()
  {
    parent::__construct();

    $this->load->model('Putusan_expedited_model', 'data_model');
  }
 
  public function index()
  {
    $data['title'] = APPNAME.' - Putusan Telaah Expedited';
    $data['page_header'] = 'Daftar Putusan Telaah Expedited';
    $data['breadcrumb'] = 'Putusan Telaah Expedited';
    $data['css_content'] = 'putusan_expedited_view_css';
    $data['main_content'] = 'putusan_expedited_view';
    $data['js_content'] = 'putusan_expedited_view_js';
  ...
  public function form($id=0, $id_pep=0)
  {
    ...
    $data['title'] = APPNAME.' - Putusan Telaah Expedited';
    $data['page_header'] = $pengajuan['revisi_ke'] > 0 ? 'Form Putusan Telaah Expedited (Perbaikan)' : 'Form Putusan Telaah Expedited';
    $data['breadcrumb'] = 'Putusan Telaah Expedited';
    $data['css_content'] = 'putusan_expedited_form_css';
    $data['main_content'] = 'putusan_expedited_form';
    $data['js_content'] = 'putusan_expedited_form_js';
  ...
  public function validation_form()
  {
    ...
    if ($this->input->post('keputusan') == 'F' && ($this->session->userdata('id_group_'.APPAUTH) == 4 || $this->session->userdata('id_group_'.APPAUTH) == 7 || $this->session->userdata('id_group_'.APPAUTH) == 8)){
      $this->form_validation->set_rules('penelaah_mendalam', 'Penelaah Mendalam', 'callback_check_penelaah');
      $this->form_validation->set_rules('pelapor_telaah', 'Pelapor', 'trim|required');
    }
  ...
  {
    $pe = $this->input->post('penelaah_mendalam');
    ...
    if ($jml_pe + $jml_lp < 5) 
    { 
      $this->form_validation->set_message('check_penelaah', 'Penelaah Mendalam Full Board minimal berjumlah 5 penelaah & lay person');
      return FALSE;
    } 
    else 
    { 
  ...
  function get_protokol()
  {
    ...
    if ($result){
      for($i=0; $i<count($result); $i++){
        $response->data[] = array(
          'id' => isset($result[$i]['id_pexp']) ? $result[$i]['id_pexp'] : 0, // untuk halaman sekretaris
          'id_pep' => $result[$i]['id_pep'],
  ...
  function get_penelaah_mendalam_by_param($id_pep, $id_paef)
  {
    $result = $this->data_model->get_data_penelaah_mendalam_by_param($id_pep, $id_paef);
  ...
  function get_pelapor_by_param($id_pep, $id_paef)
  {
    $result = $this->data_model->get_data_pelapor_by_param($id_pep, $id_paef);

    $response = (object) NULL;
    $response->id_atk = 0;
    if ($result)
      $response->id_atk = $result['id_atk_pelapor'];

    echo json_encode($response);
  }

  function get_lay_person_by_param($id_pep, $id_paef)
  {
    $result = $this->data_model->get_data_lay_person_by_param($id_pep, $id_paef);
  ...
  function get_konsultan_by_param($id_pep, $id_paef)
  {
    $result = $this->data_model->get_data_konsultan_by_param($id_pep, $id_paef);
...
/*  public function hapus()
  ...
  public function cetak_telaah_expedited($id_texp=0, $id_pep=0)
  {
    $this->load->library('Pdf');
    ...
    $pdf->SetCreator(PDF_CREATOR);
    $pdf->SetAuthor('E-Protokol');
    $pdf->SetTitle('Telaah Expedited');
    $pdf->SetSubject('Telaah Expedited');
    ...
    $data_telaah = $this->data_model->get_data_telaah_expedited_by_id_texp($id_texp);
    ...
    if (isset($data_telaah['kelayakan']))
    {
      switch($data_telaah['kelayakan'])
      {
        ...
        case 'F': $kelayakan = 'Full Board'; break;
        default: $kelayakan = '';
      }
    }

    $pdf->writeHTML('<h1>Telaah Expedited</h1><br/>', true, false, true, false, 'C');
    ...
    //Close and output PDF document
    $pdf->Output('telaah-expedited.pdf', 'I');
  }
