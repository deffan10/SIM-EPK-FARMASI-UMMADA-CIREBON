    ...
    <script type="text/javascript">
        ...
        $('#kendala').html("<?php echo isset($data['kendala']) ? $data['kendala'] : ''?>");
    	});
    	...
    	var MonevModel = function(){
    		var self = this;
    		self.id = ko.observable(<?php echo isset($data['id_monev']) ? $data['id_monev'] : 0 ?>);
    	...
    	App.do_upload = function(){
    		...
    		$.ajax({
    		    url: '<?php echo base_url()?>monev/do_upload',
    		    type: 'post',
    	...
    	App.back = function(){
    		window.location.href = '<?php echo base_url()?>monev/';
    	}
    	...
    	App.init_fileupload = function(){
    		if (App.id() > 0)
    		{
    			...
    			$.ajax({
    				url: '<?php echo base_url()?>monev/get_fileupload_by_id/'+id,
    				type: 'post',
    				dataType: 'json',
    				success: function(res, xhr){
    	...
    	App.save = function(createNew){
    	  ...
    	  App.processing(true);
    	  $.ajax({
    	    url: '<?php echo base_url()?>monev/proses',
    	    type: 'post',
    	    cache: false,
    	    dataType: 'json',
    	    data: data,
    ...
    <?php if (isset($data['id_monev']) && $data['id_monev'] > 0) { ?>
    	App.init_fileupload();
    <?php } ?>
