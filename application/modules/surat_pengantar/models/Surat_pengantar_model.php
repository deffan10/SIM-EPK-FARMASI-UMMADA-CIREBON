...
class Surat_pengantar_model extends Core_Model {
  ...
  var $fieldmap_filter;
	...
	function insert_sp()
	{
		if (isset($this->data['id_sp']) && $this->data['id_sp'] > 0)
		{
			$this->db->where('id_sp', $this->data['id_sp']);
			$this->db->update('tb_surat_pengantar', $this->data);
			$this->check_trans_status('update tb_surat_pengantar failed');
		}
		else
		{
			unset($this->data['id_sp']);
			$this->db->insert('tb_surat_pengantar', $this->data);
			$this->check_trans_status('insert tb_surat_pengantar failed');
		}		
	}

  public function get_data_jqgrid($param, $isCount=FALSE, $CompileOnly=False)
  {
    $this->db->select('sp.id_sp, sp.id_pengajuan, sp.nomor, sp.tanggal, p.no_protokol, p.judul');
    $this->db->from('tb_surat_pengantar as sp');
    $this->db->join('tb_pengajuan as p', 'p.id_pengajuan = sp.id_pengajuan');
	...
	{
		$this->db->select('sp.id_sp, sp.id_pengajuan, sp.nomor, sp.tanggal, sp.file_name, sp.client_name, sp.link_gdrive, p.no_protokol, p.judul');
		$this->db->from('tb_surat_pengantar as sp');
    $this->db->join('tb_pengajuan as p', 'p.id_pengajuan = sp.id_pengajuan');
		$this->db->where('sp.id_sp', $id);
    ...
    $this->db->join('tb_surat_pengantar as sp', 'sp.id_pengajuan = p.id_pengajuan', 'left');
		$this->db->where('p.id_pengusul', $this->session->userdata('id_pengusul'));
		$this->db->where('sp.id_sp is null');
  ...
  function check_exist_data($id)
	...
	function delete_sp($id)
	{
		...
		$this->db->delete('tb_surat_pengantar');
	}
